...
usage () {
  ...
  echo "PROMETHEUS_APPNAME=example-prometheus" 1>&2
...
echo "${red}Warning: This will delete all data for $PROMETHEUS_APPNAME in namespace $NAMESPACE. ${white}"
if [ ! -z $REMOVE_NAMESPACE ]; then
  echo "${red}It will also remove the namespace $NAMESPACE, unless other resources still exist. ${white}"
fi
echo -n "${yellow}Type the APPNAME ($PROMETHEUS_APPNAME) to continue: ${white}"
read CHECKNAME
if [ ${CHECKNAME} != $PROMETHEUS_APPNAME ]; then
