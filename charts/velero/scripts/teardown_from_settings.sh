...
echo "${red}Warning: This will delete all data for $VELERO_APPNAME in namespace $NAMESPACE. ${white}"
if [ ! -z $REMOVE_NAMESPACE ]; then
  echo "${red}It will also remove the namespace $NAMESPACE, unless other resources still exist. ${white}"
fi
echo -n "${yellow}Type the APPNAME ($VELERO_APPNAME) to continue: ${white}"
read CHECKNAME
if [ ${CHECKNAME} != $VELERO_APPNAME ]; then
...
echo "${yellow}Deleting resources... ${white}"
echo "${magenta}helm delete $VELERO_APPNAME --namespace $NAMESPACE ${white}"
helm delete $VELERO_APPNAME --namespace $NAMESPACE
echo ""
